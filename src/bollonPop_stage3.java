...
public class bollonPop_stage3 extends GameEngine {
    public static void main(String[] args) {
        createGame(new bollonPop_stage3());
    }
    ...
    public void update(double dt) {
        ...
        for (int i = 0;i < num;i++){
            if ((positionY.get(i) > height()) && visible.get(i) && (color.get(i) == green)) {
                score -= 10;
                visible.set(i,false);
            }
        }
    ...
    public void paintComponent() {
        ...
        changeColor(white);
        drawText(50,50,"Score:"+score+" Time:"+String.format("%.2f",time),"Arial",10);
        ...
        if (gameOver){
            changeColor(orange);
            drawText(100,250,"GAME OVER","Arial",50);
            drawText(100,300,"Time="+String.format("%.2f",time),"Arial",20);
            changeColor(yellow);
            drawText(100,350,"Press Space to Restart","Arial",20);
            if(space) reset();
        }
